...
const FAQs = () => {
    ...
    return (
        <section className="bg-black py-12 md:py-24 relative overflow-hidden" id="faq">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-[0.02] select-none z-0">
                <span className="text-[25vw] font-black uppercase text-white leading-none tracking-tighter">Inquiry</span>
            </div>
            ...
            <div className="container-wide relative z-10">
                <div className="max-w-4xl mx-auto text-center mb-16 space-y-4 anim-section-reveal">
                    ...
                    <h2 className="text-4xl md:text-7xl font-black uppercase tracking-tighter leading-[0.85]">
                        Common <span className="text-transparent" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)' }}>Inquiries</span>
                    </h2>
                    <p className="text-white/30 text-[0.65rem] font-bold uppercase tracking-widest max-w-xl mx-auto italic">
                ...
                <div className="max-w-3xl mx-auto space-y-2">
                    {faqs.map((faq, idx) => (
                        <div key={idx} className="group border border-white/5 bg-charcoal/20 hover:bg-white/[0.02] transition-all duration-700 anim-section-reveal">
                            <button 
                                onClick={() => setOpenIdx(openIdx === idx ? null : idx)}
                                className="w-full flex justify-between items-center p-6 md:p-8 text-left group"
                            >
                                <div className="flex items-center gap-6">
                                    <span className="text-gold/20 text-[0.5rem] font-black italic group-hover:text-gold transition-colors">0{idx + 1}</span>
                                    <span className={`text-base md:text-lg font-black uppercase tracking-tight transition-colors duration-500 ${openIdx === idx ? 'text-gold' : 'text-white/60 group-hover:text-white'}`}>
                                        {faq.q}
                                    </span>
                                </div>
                                <div className={`w-8 h-8 rounded-full border border-white/10 flex items-center justify-center transition-all duration-500 ${openIdx === idx ? 'bg-gold border-gold text-black rotate-45' : 'group-hover:border-gold/40'}`}>
                            ...
                            <div className={`overflow-hidden transition-all duration-700 ease-in-out ${openIdx === idx ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
                                <div className="px-8 md:px-14 pb-8">
                                    ...
                                    <p className="text-ivory/40 text-sm md:text-base italic leading-relaxed font-light tracking-wide max-w-2xl">
                                        {faq.a}
                                    </p>
                                </div>
                            </div>
                ...
                <div className="mt-16 text-center anim-section-reveal">
                    <p className="text-[0.45rem] uppercase tracking-[0.5em] text-white/20 font-black mb-6">Still Have Questions?</p>
                    <a href="#contact" className="px-12 py-5 border border-gold/40 text-gold text-[0.6rem] font-black uppercase tracking-[0.4em] hover:bg-gold hover:text-black transition-all duration-500 no-underline">
                        Secure Inquiry Line
                    </a>
                </div>
            </div>
