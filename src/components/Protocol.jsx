...
const Protocol = () => {
    ...
    return (
        <section className="bg-charcoal py-12 md:py-24 relative overflow-hidden" id="protocol">
            {/* Background Decoration */}
            <div className="absolute top-0 right-0 w-full h-full pointer-events-none opacity-[0.02] select-none">
                <span className="text-[30vw] font-black uppercase text-white leading-none absolute -bottom-10 -right-10">Empire</span>
            </div>
            ...
            <div className="container-wide relative z-10">
                <div className="max-w-4xl mx-auto text-center mb-16 space-y-4 anim-section-reveal">
                    ...
                    <h2 className="text-4xl md:text-7xl font-black uppercase tracking-tighter leading-[0.85]">
                        How It <span className="text-transparent" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)' }}>Works</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-1">
                    {steps.map((step, idx) => (
                        <div key={idx} className="group relative p-10 md:p-14 bg-black/20 border border-white/5 hover:bg-gold/5 transition-all duration-1000 text-center anim-section-reveal">
                            <div className="relative z-10 space-y-6">
                                ...
                                <div className="flex justify-center mb-8">
                                    <div className="w-16 h-16 rounded-full border border-gold/20 flex items-center justify-center group-hover:border-gold transition-all duration-1000">
                                        <span className="text-gold text-2xl font-black italic">0{idx + 1}</span>
                                    </div>
                                </div>

                                <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter leading-none group-hover:italic transition-all duration-700">
                                    {step.title}
                                </h3>
                                
                                <p className="text-white/40 text-sm italic leading-relaxed group-hover:text-white/60 transition-colors duration-700">
                                    {step.desc}
                                </p>
                            ...
                            {idx < steps.length - 1 && (
                                <div className="absolute right-0 top-1/2 -translate-y-1/2 h-1/2 w-px bg-white/5 hidden md:block"></div>
                            )}
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-16 text-center anim-section-reveal">
                    <div className="inline-flex items-center gap-6">
                        ...
                        <span className="text-white/20 text-[0.45rem] font-black uppercase tracking-[0.6em]">Verified Protocol / Est. 2012</span>
                        <div className="w-12 h-px bg-white/10"></div>
                    </div>
                </div>
            </div>
